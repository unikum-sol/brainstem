...
def guard_active(name):
    """Public, non-underscored accessor for other modules (e.g. the guarded
    dual-compute comparator) that need to know whether kernel-guarded mode is
    explicitly enabled for a given kernel name, without reaching into the
    private _mode() helper directly. Default authority remains "old" unless
    explicitly overridden via the BRAINSTEM_KERNEL_AUTH_<NAME> environment
    variable, exactly as before."""
    return _mode(name) == "kernel_guarded"
def _finite(x):
