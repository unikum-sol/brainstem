...
from ki_system import v8_modern_projection_contract_release as modern_projection_contract
...
SCHEMA_TABLES = {OBS_TABLE: ['observation_key', 'shadow_key', 'hypothesis_id', 'source_updated_at', 'center_chunk_id', 'target_chunk_ids', 'target_count', 'projected_window_strategy', 'projected_window_radius', 'projected_action', 'expected_gain', 'closure_delta', 'overlap_score', 'read_no_candidate_rate', 'projected_effectiveness', 'neuromodulators', 'real_outcome_observation_available', 'observation_ready', 'source_default_path', 'productive_gap_id', 'productive_write', 'details', 'created_at', 'updated_at'], CYCLE_TABLE: ['id', 'source_rows_seen', 'observations_created', 'observations_updated', 'projection_errors', 'checkpoint_updated_at_before', 'checkpoint_hypothesis_id_before', 'checkpoint_updated_at_after', 'checkpoint_hypothesis_id_after', 'productive_writes_before', 'productive_writes_after', 'safety_ok', 'details', 'created_at'], STATE_TABLE: ['key', 'value']}
...
# BRAINSTEM_SHADOW_CONN_PASSTHROUGH_FIX_V1: the original _con()/_db_path()
# helpers never checked whether "obj" was already a live sqlite3.Connection.
# When db_bootstrap.py started passing its own already-open connection into
# ensure_schema(con) (to guarantee the shadow tables land in the SAME
# database file that is actually being bootstrapped, instead of silently
# falling back to DEFAULT_DB), this module ignored that connection, opened a
# brand new one to DEFAULT_DB, and -- because the caller's connection still
# held an open write transaction on the very same file -- deadlocked on the
# SQLite file lock for the full 120s timeout. Only ensure_schema() is fixed
# here (via a small ownership-tracking wrapper); every other function in
# this module already receives a live connection directly as its first
# parameter and was never affected.
...
def _count(con, table):
    return int(con.execute('SELECT COUNT(*) FROM ' + _q(table)).fetchone()[0]) if _table_exists(con, table) else 0
...
def ensure_schema(obj=None):
    con, _brainstem_owns_conn = _con_with_ownership(obj)
    try:
        ...
        for k, v in defaults.items():
            con.execute('INSERT OR IGNORE INTO ' + STATE_TABLE + '(key,value) VALUES(?,?)', (k, v))
        con.commit()
        return _self_check_schema(con)
    finally:
        if _brainstem_owns_conn:
            con.close()
...
def _self_check_schema(con=None):
    ...
    try:
        ...
        if not result['overall']:
            raise ShadowProjectionError('schema self-check failed: ' + repr(result))
        return result
    finally:
        if own:
            con.close()
...
def observe_shadow(obj=None, limit=MAX_BATCH):
    ...
    try:
        ...
        cp_time = int(state.get('checkpoint_updated_at') or 0)
        cp_id = int(state.get('checkpoint_hypothesis_id') or 0)
        ...
        source_hash = modern_projection_contract.contract_source_hash()
        ...
        for row_index, row in enumerate(rows):
            try:
                item = modern_projection_contract.evaluate_projection(con, row, source_hash)
                ...
                v2_stats['inputs'] += 1
                for v2_key in ('latest_insert', 'latest_update', 'version_insert', 'identical_retry'):
                    v2_stats[v2_key] += int(v2_result.get(v2_key, 0))
                if exists:
                    updated += 1
                else:
                    created += 1
        ...
        if checkpoint_row is not None:
            after_cp_time = int(checkpoint_row.get('updated_at') or 0)
            after_cp_id = int(checkpoint_row.get('hypothesis_id') or 0)
        after = _protected(con)
        protected_ok = before == after
        if not protected_ok:
            raise ShadowProjectionError('protected counts changed')
        ...
        for k, v in {'checkpoint_updated_at': after_cp_time, 'checkpoint_hypothesis_id': after_cp_id, 'last_source_rows_seen': len(rows), 'last_observations_created': created, 'last_observations_updated': updated, 'last_projection_errors': errors, 'last_safety_ok': 'true' if safety else 'false', 'observation_ready': 'false'}.items():
            _write_kv(con, k, v)
        con.commit()
    ...
    try:
        for table, cols in SCHEMA_TABLES.items():
            if table == STATE_TABLE:
                con.execute('CREATE TABLE ' + table + '(key TEXT PRIMARY KEY,value TEXT)')
            elif table == OBS_TABLE:
                con.execute('CREATE TABLE ' + table + '(observation_key TEXT PRIMARY KEY,' + ','.join((c + ' TEXT' for c in cols[1:])) + ')')
            else:
                con.execute('CREATE TABLE ' + table + '(id INTEGER PRIMARY KEY,' + ','.join((c + ' TEXT' for c in cols[1:])) + ')')
...
if __name__ == '__main__':
    if '--selftest' in sys.argv:
        selftest()
    elif '--ensure-schema' in sys.argv:
        print(json.dumps(ensure_schema(), indent=2))
    elif '--observe' in sys.argv:
        print(json.dumps(observe_shadow(), indent=2))
