...
BRAINSTEM_INDEX_DEDUP_FIX_V1: this tool used to hard-code its own copy of the
performance-index specification (INDEX_SPECS), which was a near-duplicate of
db_bootstrap.py's _PERF_INDEXES / ensure_perf_indexes(). Two independent lists
describing the same intent could silently drift apart after a future schema
change (one place updated, the other forgotten). This module is now a thin
CLI/dry-run/backup wrapper around the single canonical definition that lives
in db_bootstrap.py; db_bootstrap remains authoritative and is also what runs
automatically on every startup via ensure_perf_indexes().
"""
import sqlite3, os, shutil, sys, time
from pathlib import Path

from ki_system.db_bootstrap import _PERF_INDEXES as INDEX_SPECS
...
def _main():
    ...
    print("\nErstelle Backup (kann bei grossen Datenbanken einen Moment dauern) ...")
    shutil.copy2(db, bak)
    print("Backup:", bak)
