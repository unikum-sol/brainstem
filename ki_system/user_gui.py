...
class UserApp(tk.Tk):
    def __init__(self,db_path='ki_memory.sqlite3'):
        super().__init__(); self.title('KI-System V8 User (Read-Only)'); self.geometry('1000x700'); self.mem=Memory(db_path,readonly=True); self.dialogue=DialogueManager(self.mem); self._ui()
    def _ui(self):
        ...
        # BRAINSTEM_USER_GUI_READONLY_NOTE_V1: make the read-only nature of
        # this session explicit in the UI instead of silently dropping every
        # conversation-persist attempt (see dialogue.py fix).
        ttk.Label(chat, text='Read-Only Sitzung: Chatverlauf wird in dieser Ansicht NICHT dauerhaft gespeichert.',
                  foreground='#8a5a00').pack(anchor=tk.W, pady=(0,4))
